...
<form class="clform-options" action="<? echo($APPLICATION->GetCurPage()); ?>?mid=<? echo($module_id); ?>&lang=<? echo(LANG); ?>" method="post">
	<div class="row">
		<h2><?php echo GetMessage('CLIENT_FORM_OPTIONS_TITLE'); ?></h2>
			<label>
				<?php echo GetMessage('CLIENT_FORM_IBLOCK_ID_TITLE'); ?>
				<input name="iblock_id" type="number" value="<?php echo htmlspecialcharsbx($OPTIONS['iblock_id']); ?>" />
			</label>
	</div><!-- /.row -->
	<div class="row">
		<h2><?php echo GetMessage('CLIENT_FORM_RE_TITLE'); ?></h2>
			<a target="_blank" href="https://www.google.com/recaptcha/admin/create#list"><?php echo GetMessage('CLIENT_FORM_RE_REG_LINK'); ?></a>
			<label>
				<?php echo GetMessage('CLIENT_FORM_RE_SITE_KEY'); ?>
				<input name="RE_SITE_KEY" type="text" value="<?php echo htmlspecialcharsbx($OPTIONS['RE_SITE_KEY']); ?>" />
			</label>
			<label>
				<?php echo GetMessage('CLIENT_FORM_RE_SEC_KEY'); ?>
	...
	$tabControl->Buttons();
	?>
	<input type="submit" name="apply" value="<? echo(Loc::GetMessage("CLIENT_FORM_BTN_APPLY")); ?>" class="adm-btn-save" />
</form>
<br />
...
<div class="help-block adm-detail-content-wrap">
	<?php echo GetMessage('CLIENT_FORM_EVENTS_DESCRIPTION'); ?>
</div><!-- /.help-block -->
