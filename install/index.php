...
class collected_clientforms extends CModule
{
	...
	function __construct()
	{
		...
		$this->MODULE_NAME = GetMessage("CLIENT_FORMS_NAME");
		$this->MODULE_DESCRIPTION = GetMessage("CLIENT_FORMS_DESC");
		$this->PARTNER_NAME = GetMessage("CLIENT_FORMS_PARTNER_NAME");
		$this->PARTNER_URI = GetMessage("CLIENT_FORMS_PARTNER_URI");
	}
	...
	public function AddIblock($arFieldsIB){
		...
		$arFieldsIB['NAME'] = GetMessage("CLIENT_FORMS_IBLOCK_NAME_RU");
	...
	public function createIblocks(){
		$arFieldsForType = Array(
			...
			'NAME'=> GetMessage("CLIENT_FORMS_IBLOCK_NAME_RU"),
			'SECTIONS' => 'N',
			'IN_RSS' => 'N',
			'SORT' => 500,
			'LANG' => Array(
				'en' => Array(
					'NAME' =>  GetMessage("CLIENT_FORMS_IBLOCK_NAME_EN"),
				),
				'ru' => Array(
					'NAME' => GetMessage("CLIENT_FORMS_IBLOCK_NAME_RU"),
				)
		...
		if ($this->AddIblockType($arFieldsForType)!=''){
			$arFieldsForIblock = Array(
				"ACTIVE" => "Y",
				"NAME" => GetMessage("CLIENT_IBLOCK_NAME"),
			...
			CAdminMessage::ShowMessage(Array(
				"TYPE" => "ERROR",
				"MESSAGE" => GetMessage("CLIENT_IBLOCK_CREATE_ERR"),
				"DETAILS" => "",
				"HTML" => true
			));
		}
	...
	function DoInstall()
	{
		...
		if (is_array($this->NEED_MODULES) && !empty($this->NEED_MODULES))
			foreach ($this->NEED_MODULES as $module)
				if (!IsModuleInstalled($module))
					$this->errors[] = array('ERROR', GetMessage('CLIENT_FORMS_INSTALLER_NEED_MODULES', array('#MODULE#' => $module)));
		...
		if (strlen($this->NEED_MAIN_VERSION) <= 0 || version_compare(SM_VERSION, $this->NEED_MAIN_VERSION) >= 0)
		{
			...
			$this->errors[] = array('OK', GetMessage('CLIENT_FORMS_INSTALLER_INSTALL_OK'), GetMessage('CLIENT_FORMS_INSTALLER_GOTO_SETTINGS_BUTTON'), '/bitrix/admin/settings.php', array('mid' => $this->MODULE_ID, 'mid_menu' => 1));
		}
		else
			$this->errors[] = array('ERROR', GetMessage('CLIENT_FORMS_INSTALLER_NEED_RIGHT_VER', array('#NEED#' => $this->NEED_MAIN_VERSION)));
		
		$GLOBALS["errors"] = $this->errors;
		$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage('CLIENT_FORMS_NAME'), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/".$this->MODULE_ID."/install/step1.php");
	}
	...
	function InstallEvents()
	{
		...
		/* EMAIL EVENTS */
		$arET = CEventType::GetByID("CLIENT_FORM_ADD", "ru")->Fetch();
        $eID = '';
		if (!$arET)
		{
			$obEventType = new CEventType;
			$eID = $obEventType->Add(array(
					"EVENT_NAME"    => "CLIENT_FORM_ADD",
					"SORT"			=> 150,
					"NAME"          => GetMessage("CLIENT_FORMS_MAIL_EVENT_NAME"),
					"LID"       	=> "ru",
					"DESCRIPTION"   => GetMessage("CLIENT_FORMS_MAIL_EVENT_DESCRIPTION")
				));
		...
		//$rsMess = CEventMessage::GetList($by = "site_id", $order = "desc", Array("TYPE_ID" => "CLIENT_COMMENT_ADD"));
		if ($eID)
		{
			$arQuery = CSite::GetList($sort = "sort", $order = "desc", Array());
			while ($res = $arQuery->Fetch())
			...
			$arr = array(
				...
				"EMAIL_TO"		=> "#EMAIL_TO#",
				"BCC"			=> "",
				"SUBJECT"		=> GetMessage("CLIENT_FORMS_MAIL_EVENT_SUBJECT"),
				"BODY_TYPE"		=> "html",
				"MESSAGE"		=> GetMessage("CLIENT_FORMS_MAIL_EVENT_MESSAGE")
			);
		...
		RegisterModuleDependences("main", "OnPanelCreate", self::MODULE_ID, "ClientForms", "insertAdminScripts", 1);
		RegisterModuleDependences("main", "OnPageStart", self::MODULE_ID, "ClientForms", "insertGlobalScripts", 1);
	...
	{
		UnRegisterModuleDependences("main", "OnPanelCreate", self::MODULE_ID, "ClientForms", "insertAdminScripts");
		UnRegisterModuleDependences("main", "OnPageStart", self::MODULE_ID, "ClientForms", "insertGlobalScripts");
